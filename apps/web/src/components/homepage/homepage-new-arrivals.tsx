...
export function HomepageNewArrivals() {
  ...
  useEffect(() => {
    fetchProducts({ limit: 6, sort: "newest", isActive: true })
    ...
    <section className="w-full max-w-7xl md:max-w-4/5 mx-auto px-4 md:px-0 py-8 sm:py-12 bg-white font-sans">
      <div className="flex items-center justify-between pb-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          <Link href="/new-arrivals" className="text-xs font-bold text-[#15803d] hover:text-emerald-800 transition underline underline-offset-4 mr-1">
            View All New Arrivals
          </Link>
          <button className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 flex items-center justify-center transition cursor-pointer" aria-label="Previous New Arrival">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 flex items-center justify-center transition cursor-pointer" aria-label="Next New Arrival">
              ...
              return (
                <div key={product.id} className="group bg-white border border-slate-200/80 hover:border-slate-300 rounded-xl p-3.5 flex flex-col justify-between transition-all duration-300 shadow-2xs hover:shadow-md relative">
                  <div className="flex items-center justify-between w-full mb-2 h-5">
                    <span className="bg-emerald-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider">NEW</span>
                  </div>
