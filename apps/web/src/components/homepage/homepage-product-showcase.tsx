      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-3 mb-6">
        <div className="flex items-center gap-6 sm:gap-8 overflow-x-auto no-scrollbar">
          {loadingCats
            ? [...Array(3)].map((_, i) => (
                <div
                  key={i}
                  className="w-28 h-5 bg-slate-100 rounded animate-pulse"
                />
              ))
            : categories.map((cat) => {
        ...
        <div className="flex items-center gap-2 self-end sm:self-center">
          <button
            ...
            aria-label={tHome("prevCategory")}
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button
            className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 flex items-center justify-center transition cursor-pointer rtl:rotate-180"
            aria-label={tHome("nextCategory")}
          >
                  ...
                  >
                    {primaryImage?.url ? (
                      <img
                        ...
                        alt={product.name}
                        className={`w-full h-full object-cover rounded ${!inStock ? "opacity-35" : ""}`}
                      />
                    ) : (
